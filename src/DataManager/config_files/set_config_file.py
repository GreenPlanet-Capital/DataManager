...
assetConfigFileName = os.path.join(
    os.path.dirname(inspect.getfile(config_files)), "assetConfig.cfg"
)
...
def gdrive_client_secrets(secrets):
    clientSecretsFilePath = os.path.join(
        os.path.dirname(inspect.getfile(config_files)), "client_secrets.json"
    )
...
def set_keys(
    section: str,
    public_key_var_name: str,
    public_key: str,
    private_key_var_name: str,
    private_key: str,
):
    config = configparser.ConfigParser()
    config.read(assetConfigFileName)
    ...
    if section in config.sections():
        return (
            False,
            f"Section {section} already exists in the config file\n\
