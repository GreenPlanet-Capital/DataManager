...
ENV DATABASE_REMOTE="https://doltremoteapi.dolthub.com/post-no-preference/options"
ENV DATABASE_NAME=options
ENV DOLTHUB_USER=post-no-preference
ENV DOLTHUB_EMAIL=arnav.nidumolu@gmail.com
ENV DATA_DIR="/dolthub-dbs/post-no-preference/$DATABASE_NAME"
...
create_directories() {
  ...
  _create_dir "/dolthub-dbs/post-no-preference"
}
...
clone_database() {
  ...
  echo "Cloning database..."
    (
      local wd=\$(pwd)
      cd "/dolthub-dbs/post-no-preference"
...
WORKDIR /dolthub-dbs/post-no-preference
CMD [ "dolt", "sql-server", "-l=trace", "--host=0.0.0.0", "--port=6000"]
